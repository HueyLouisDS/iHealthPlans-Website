...
--
-- Every row in leads has a matching row in lead_consents, enforced by a
-- foreign key, and /api/leads/inbound refuses any lead that arrives without
-- the evidence. That guarantee is the single most valuable property of that
-- table. It is what lets the agency answer an audit with a query instead of an
-- email thread.
--
